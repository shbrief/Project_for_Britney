...
bin_fac <- binary_factor[,-c(5, 12, 26)]
...
graph_categorical <- function(rav1, rav2, c, d, e = 'Legend') {
  ...
  plot_data <- data.frame(plot_data, d)
  ...
  pA <- ggplot(plot_data_2,
        ...
        geom_point() +
        #ggtitle('Score Plot for Gender') +
        scale_color_manual(values = colors.toplot, name = e) +
        theme_bw() +
        theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank()) +
        geom_hline(yintercept = 0, linetype = 'dashed') +
...
```{r warning=FALSE}
graph_categorical(834, 833, 'pathology_M_stage', new_factorTb, 'Pathology M Stage')

graph_categorical(834, 1302, 'pathology_M_stage', new_factorTb, 'Pathology M Stage')
```

```{r warning=FALSE}
graph_categorical(834, 517, 'patient.microsatellite_instability', new_factorTb, 'Microsatellite Instability')
```
```{r warning=FALSE}
graph_categorical(834, 1302, 'patient.microsatellite_instability_test_results.microsatellite_instability_test_result.mononucleotide_and_dinucleotide_marker_panel_analysis_status', new_factorTb, 'Microsatellite Instability')
```

```{r warning=FALSE}
graph_categorical(832, 833, 'race', new_factorTb, 'Race')
```
