...
```{r}
validated_ind <- validatedSignatures(validate_coad_rna, num.out = 500, RAVmodel, swCutoff = 0.01, indexOnly = TRUE)
...
for (i in 1:length(factorTb)) {
  if (any(is.na(levels(factorTb[,10])))) {
    if (nlevels(factorTb[,i]) == 3) {
      nominal_ind <- c(nominal_ind, i)
    } else if (nlevels(factorTb[,i]) > 3) {
      factor_ind <- c(factor_ind, i)
    }
  } else {
    if (nlevels(factorTb[,i]) > 2) {
      factor_ind <- c(factor_ind, i)
    }
  }
}
...
nominalTb <- factorTb[,nominal_ind]
```
...
plot_data <- sampleScore[, c("RAV1001", "RAV1002")]
