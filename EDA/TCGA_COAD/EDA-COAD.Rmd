...
```{r}
t_test_res <- as.data.frame(matrix(nrow = ncol(binary_factor),
                                ncol = ncol(sampleScore_sub)))

rownames(t_test_res) <- colnames(binary_factor)
colnames(t_test_res) <- colnames(sampleScore_sub)

ttest_coad_tvalue <- t_test_res
ttest_coad_pvalue <- t_test_res

t.test(sampleScore_sub[, 1] ~ binary_factor[, 9], na.action = na.omit)

binary_factor[, 9]

for (i in seq_len(ncol(sampleScore_sub))) {
  for (j in seq_len(ncol(binary_factor))) {
    
    ## ANOVA
    t_test <- t.test(sampleScore_sub[, i] ~ binary_factor[, j], na.action = na.omit)
    
    ## F-statistic
   ## fval <- t_test[[1]]
    ##ttest_coad_tvalue[j, i] <- fval
...
```
```{r warning=FALSE}
graph_categorical(834, 517, 'patient.microsatellite_instability_test_results.microsatellite_instability_test_result.mononucleotide_and_dinucleotide_marker_panel_analysis_status', 'Microsatellite')

```

```{r warning=FALSE}
graph_categorical(832, 833, 'race', 'Race')
```
