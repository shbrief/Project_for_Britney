...
plot_data <- sampleScore_sub[, c("RAV4689", "RAV1959")]
colnames(plot_data)[1] <- "sampleScore1"
colnames(plot_data)[2] <- "sampleScore2"
...
rav_exploration <- function(indices) {
  for (x in 1:length(indices)) {
    print(findStudiesInCluster(RAVmodel, indices[x], studyTitle = TRUE))
    drawWordcloud(RAVmodel, indices[x])
  }
}

rav_exploration(indices)
...
```{r warning=FALSE}
graph_categorical(2673, 998, 'pathology_N_stage', 'Pathology N Stage')
graph_categorical(998, 1959, 'patient.primary_lymph_node_presentation_assessment')
graph_categorical(1125, 3033, 'patient.drugs.drug.2.therapy_types.therapy_type')
```
